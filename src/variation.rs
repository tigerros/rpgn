...
impl Variation {
    ...
    pub fn new_variation_at_last_move(&self, moves_capacity: usize) -> Self {
        if let Some(last_move_number_sub1) = self.last_move_number().index.checked_sub(1) {
            if let Some(moves_len_sub2) = self.moves.len().checked_sub(2) {
            ...
            Self::new_root_variation(MoveNumber { index: 0 }, self.starting_position.clone(), moves_capacity)
        }
    }
